 ...
 * ({@link FlavorListener}). and when detected it will set a trigger for the
 * service thread to check and process, thus note there might be a delay between
 * the time the clipboard changes and the time the thread is able to process the
 * change.
 * <br>
 * NOTE: this class does NOT process the clipboard changes in the EDT, this is
 * intentional as per design, as working with the EDT would cause irresponsive
 * UI, or delays
 * <br>
 * this service by itself does not handle the data. the interested need to
 * register a {@link FlavorProcessor} and register using 
 ...
 * once register the service will call this interface to handle the clipboard
 * data.
 * <br>
 * NOTE: a Clipboard Event will be consider handled when the first possible
 * Handler reports that it has successfully handled the event. and no other
 * listener will be called, it allow to register multiple handlers. (sort of a
 * hybrid Unicast event)
 *
 * @see FlavorListener
 * @see FlavorProcessor
 * @since 1.2
...
public final class ClipboardService implements Runnable, FlavorListener, ClipboardOwner {
    ...
    private ClipboardService() {
        if (SYSTEM_CLIPBOARD == null) {
            LoggingHelper.getLogger(LOGGERNAME).log(Level.SEVERE, "Error Headeless instance");
    ...
    public synchronized boolean StartClipBoardService() {
        LoggingHelper.getLogger(LOGGERNAME).entering("ClipboardService", "StartClipBoardService()");
        ...
        LoggingHelper.getLogger(LOGGERNAME).info("Service is going to start, resetting vars.");
        resetValues();
        /**
         ...
         * possible.
         */
        LoggingHelper.getLogger(LOGGERNAME).info("Starting the Service Thread");
        serviceOnline = true;
    ...
    public synchronized boolean StopClipBoardService() {
        ...
        LoggingHelper.getLogger(LOGGERNAME).info("Requesting Service To stop");
        serviceOnline = false;
    ...
    public synchronized boolean StopClipBoardProcessing() {
        ...
        LoggingHelper.getLogger(LOGGERNAME).info("Requesting to Stop Processing Data");
        processingData = false;
        return true;
    }
    //</editor-fold>
    ...
    public void run() {
        ...
        while (serviceOnline) {
            synchronized (this) {
                ...
                if (RequestQueue.isEmpty() && !forceProcess) {
                    try {
                        LoggingHelper.getLogger(LOGGERNAME).info("Waiting for Work");
                        this.wait();
        ...
        LoggingHelper.getLogger(LOGGERNAME).info("Leaved the Main Clipboard Service Loop, Shutting Down");
    ...
    private void DebugLog(Transferable contents) {
        ...
        HashSet<String> flavorsNames = new HashSet<>();
        HashSet<String> flavorsMime = new HashSet<>();
        for (DataFlavor flavor : contents.getTransferDataFlavors()) {
            if (flavor == null) {
                continue;
            }
            var added = flavorsNames.add(flavor.getHumanPresentableName());
